<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('Edit Email Rule') }} - {{ $emailRule->name }}
        </h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <form method="POST" action="{{ route('email-rules.update', $emailRule) }}" class="space-y-6">
                        @csrf
                        @method('PUT')
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <input type="text" name="name" id="name" value="{{ old('name', $emailRule->name) }}" required class="mt-1 block w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
                                    @error('name')
                                ...
                                <div>
                                    ...
                                    <select name="email_account_id" id="email_account_id" required class="mt-1 block w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
                                        ...
                                        @foreach($emailAccounts as $account)
                                            <option value="{{ $account->id }}" {{ old('email_account_id', $emailRule->email_account_id) == $account->id ? 'selected' : '' }}>
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                <div class="border-2 {{ $emailRule->action === 'create_ticket' ? 'border-blue-500 dark:border-blue-400' : 'border-gray-200 dark:border-gray-700' }} rounded-lg p-4 hover:border-blue-500 dark:hover:border-blue-400 cursor-pointer transition-colors" onclick="selectAction('create_ticket')">
                                ...
                                <div class="border-2 {{ $emailRule->action === 'auto_respond' ? 'border-blue-500 dark:border-blue-400' : 'border-gray-200 dark:border-gray-700' }} rounded-lg p-4 hover:border-blue-500 dark:hover:border-blue-400 cursor-pointer transition-colors" onclick="selectAction('auto_respond')">
                                    <div class="flex items-center mb-3">
                                ...
                                <div class="border-2 {{ $emailRule->action === 'forward' ? 'border-blue-500 dark:border-blue-400' : 'border-gray-200 dark:border-gray-700' }} rounded-lg p-4 hover:border-blue-500 dark:hover:border-blue-400 cursor-pointer transition-colors" onclick="selectAction('forward')">
                                    <div class="flex items-center mb-3">
                            ...
                            <input type="hidden" name="action" id="action" value="{{ old('action', $emailRule->action) }}" required>
                            @error('action')
                                <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div id="action-config" class="{{ $emailRule->action === 'create_ticket' ? 'hidden' : '' }}">
                            <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-4">Action Configuration</h3>
                            
                            <!-- Auto Respond Configuration -->
                            <div id="auto-respond-config" class="{{ $emailRule->action === 'auto_respond' ? '' : 'hidden' }}">
                                <div class="space-y-4">
                                    <div>
                                        <label for="response_subject" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Response Subject</label>
                                        <input type="text" name="response_subject" id="response_subject" value="{{ old('response_subject', $emailRule->response_subject) }}" class="mt-1 block w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
                                    </div>
                                    <div>
                                        <label for="response_body" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Response Body</label>
                                        <textarea name="response_body" id="response_body" rows="6" class="mt-1 block w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">{{ old('response_body', $emailRule->response_body) }}</textarea>
                            ...
                            <div id="forward-config" class="{{ $emailRule->action === 'forward' ? '' : 'hidden' }}">
                                <div>
                                    <label for="forward_to" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Forward To</label>
                                    <input type="email" name="forward_to" id="forward_to" value="{{ old('forward_to', $emailRule->forward_to) }}" class="mt-1 block w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
                        ...
                        <div>
                            ...
                            <div id="conditions-container" class="space-y-4">
                                @if($emailRule->conditions)
                                    @foreach(json_decode($emailRule->conditions, true) as $index => $condition)
                                        <div class="grid grid-cols-1 md:grid-cols-4 gap-4 p-4 border border-gray-200 dark:border-gray-700 rounded-lg">
                                            <div>
                                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Field</label>
                                                <select name="conditions[{{ $index }}][field]" required class="mt-1 block w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
                                                    ...
                                                    <option value="has_attachment" {{ $condition['field'] === 'has_attachment' ? 'selected' : '' }}>Has Attachment</option>
                                                </select>
                                            </div>
                                            <div>
                                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Operator</label>
                                                <select name="conditions[{{ $index }}][operator]" required class="mt-1 block w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
                                                    ...
                                                    <option value="regex" {{ $condition['operator'] === 'regex' ? 'selected' : '' }}>Regex</option>
                                                </select>
                                            </div>
                                            <div>
                                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Value</label>
                                                <input type="text" name="conditions[{{ $index }}][value]" value="{{ $condition['value'] }}" required class="mt-1 block w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
                                            </div>
                        ...
                        <div>
                            ...
                            <div class="flex items-center">
                                <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $emailRule->is_active) ? 'checked' : '' }} class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded">
                        ...
                        <div class="flex justify-end space-x-3 pt-6 border-t border-gray-200 dark:border-gray-700">
                            <a href="{{ route('email-rules.show', $emailRule) }}" class="bg-gray-300 dark:bg-gray-600 hover:bg-gray-400 dark:hover:bg-gray-500 text-gray-700 dark:text-gray-300 font-medium py-2 px-4 rounded transition-colors">
                                Cancel
                            </a>
    ...
    <script>
        let selectedAction = '{{ $emailRule->action }}';
        let conditionCount = {{ $emailRule->conditions ? count(json_decode($emailRule->conditions, true)) : 0 }};
        ...
        // Initialize action selection on page load
